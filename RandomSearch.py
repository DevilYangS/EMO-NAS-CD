...
class RandomSearch():
    ...
    def Initialization(self):
        if config.Continue_path is None:
            ...
            self.gen = int(latest_file_or_folder[-2:])+1
    ...
    def Genetic_operation(self):
        ...
        for idx in range(0,self.Popsize):
            num_nodes = np.random.randint(config.Num_Nodes[0],config.Num_Nodes[1]+1) # +1
            self.offspring.append(Individual(num_Nodes=num_nodes,mapping=self.mapping,config=self.config,gen=self.gen,id=idx))
