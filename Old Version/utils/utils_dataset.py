...
def get_dataset(name = 'assist',batch_size = 32, num_workers=0):
    with open("data/{}/{}_val_set.json".format(name,name), encoding='utf8') as i1_f:
        valid_data1 = json.load(i1_f)

    with open("data/{}/{}_train_set.json".format(name,name), encoding='utf8') as i_f:
        train_data1 = json.load(i_f)

    with open("data/{}/{}_test_set.json".format(name,name), encoding='utf8') as i2_f:
        test_data1 = json.load(i2_f)

    item2knowledge = np.loadtxt("data/{}/{}_item2knowledge.txt".format(name,name))
