...
class Individual():
    ...
    def RandomBuildTree(self):
        ...
        #概率采样
        # tree.sample_prob(self.mapping,self.numNodes,self.id)
...
class EvolutionAlgorithm():
    ...
    def Initialization_from_random_and_existing(self):
            ...
            offspring_temp = []
            for idx in range(4, self.Popsize,4):
                offspring_temp.extend(Generate_from_existing(self.Population[:4],gen='initial'))

            for idx in range(4, self.Popsize):
                if np.random.rand()<0.5:
                    num_nodes = np.random.randint(config.Num_Nodes[0],config.Num_Nodes[1]+1) # +1
                    self.Population.append(Individual(num_Nodes=num_nodes,mapping=self.mapping,config=self.config,gen='initial',id=idx))
                else:
                    individual_i = offspring_temp[idx-4]
                    individual_i.id = idx
                    self.Population.append(individual_i)
    ...
    def Genetic_operation(self):
        ...
        while len(self.offspring)<self.Popsize:
            ...
            # offspring_temp = Generate_crossover_mutation(self.MatingPool,self.gen) % 消融实验， 验证 crossover + mutation
    ...
    def Main_Loop(self):
        ...
        # self.Initialization()
        # self.Initialization_from_random_and_existing()
