...
FLOPs_Dict_vecotr = {
    ...
    'add': 128,
    'mul': 128,
    'concat': 4 * 128 * 128 - 128,

    # 14: 'log',
    'mean': 128,
    'ffn_d': 2 * 128 * 128 - 128,
}
# 3-FClayer  8D^2-4D + 16D^2-2D + 4D-1
FLOPs_Dict_scalar = {
    'neg': 1,
    'abs': 1,
    'square': 1,
...
class Individual():
    ...
    def Calculate_FLOPs(self ):
        ...
        #--------------------- is Identity or 3-FC layers--------
        if self.tree.root.shape =='same':
            self.FLOPs = self.FLOPs + (4*128-1)+ (8*128*128-4*128)+(16*128*128-2*128) # 3-FC layers
            # 3-FClayer  8D^2-4D + 16D^2-2D + 4D-1
        else:
            pass # Identity, self.FLOPs = self.FLOPs+0
        ...
        while Queue:
            ...
            if cur.left is not None:
                cur_shape.append(cur.left.shape)
            if cur.right is not None:
                cur_shape.append(cur.right.shape)

            if  'same' in    cur_shape:
                self.FLOPs = self.FLOPs + FLOPs_Dict_vecotr[cur.item]
            else:
                self.FLOPs = self.FLOPs + FLOPs_Dict_scalar[cur.item]

            print(cur.item, end=' ')

            if cur.left != None and cur.left.item not in ['Stu','Exer','Conc']:
                Queue.append(cur.left)
            if cur.right != None and cur.right.item not in ['Stu','Exer','Conc']:
                Queue.append(cur.right)
...
class EvolutionAlgorithm():
        ...
        if config.Continue_path is None:
            ...
            for idx in range(0, self.Popsize):
                ...
                self.Population.append(
                    Individual(num_Nodes=num_nodes, mapping=self.mapping, config=self.config, gen='initial',
                               id=idx))
    ...
    def Initialization_from_existing(self):
        ...
        self.Population.append(Individual(Dec=[0,2,11, 3,0,8, 1,0,9, 4,5,10, 6,0,6 ],
                                          mapping=self.mapping,config=self.config,gen='initial',id=1) ) # MIRT
        ...
        # self.Population.append(Individual(Dec=[1, 0, 0],
        #                                   mapping=self.mapping, config=self.config, gen='initial', id=0))
        # self.Population.append(Individual(Dec=[1, 0, 1],
        #                                   mapping=self.mapping, config=self.config, gen='initial', id=1))
        # self.Population.append(Individual(Dec=[1, 0, 2],
        #                                   mapping=self.mapping, config=self.config, gen='initial', id=2))
        # self.Population.append(Individual(Dec=[1, 0, 3],
        #                                   mapping=self.mapping, config=self.config, gen='initial', id=3))

        for idx in range(4, self.Popsize, 4):
            offspring_temp = Generate_from_existing(self.Population[:4], gen='initial')
    ...
    def Evaluation(self, Population):
        if self.config.parallel_evaluation and self.config.n_gpu > 1:
            ...
            for i in range(0, len(Population), self.config.n_gpu):
                ...
                logging.info(
                    'solution:{0:>2d} --- {1:>2d}(Parallel evaluation)'.format(i, i + self.config.n_gpu - 1))
    ...
    def Genetic_operation(self):
        ...
        while len(self.offspring) < self.Popsize:
            ...
            for offspring_i in offspring_temp:
                if offspring_i.Dec not in self.Archive:
                    offspring_i.id = i
    ...
    def Plot(self):
        ...
        plt.ylabel('AUC')
        plt.title('Generation {0}/{1} \n best ACC: {2:.4f}, best AUC: {3:.4f}'.format(self.gen + 1, self.Maxi_Gen,
                                                                                      max(1 - self.Pop_fitness[:,
                                                                                              0]),
                                                                                      max(1 - self.Pop_fitness[:,
                                                                                              1])))
        # plt.show()
