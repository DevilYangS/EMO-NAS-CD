...
def get_dataset(config,Search = True):
    ...
    student_n,exer_n,knowledge_n,train_loader, val_loader, test_loader = eval('get_{name}_dataset'.format(name = config.dataset))(config)
    if Search:
        return student_n,exer_n,knowledge_n,train_loader, val_loader
    else:
        return student_n,exer_n,knowledge_n,train_loader, val_loader, test_loader
...
def get_junyi_dataset(config):
    ...
    return  user_n,item_n,knowledge_n,train_set, test_set, test_set
def get_slp_dataset(config):

    train_set, valid_set, test_set ,user_n,item_n,knowledge_n = get_datasetjunyi(name='slp',batch_size=config.batch_size,num_workers=config.num_workers)

    return  user_n,item_n,knowledge_n,train_set, test_set, test_set


def get_Assistment_dataset(config):
    train_data = pd.read_csv("./dataset/data/a0910/train.csv")
    valid_data = pd.read_csv("./dataset/data/a0910/valid.csv")
    test_data = pd.read_csv("./dataset/data/a0910/test.csv")
    df_item = pd.read_csv("./dataset/data/a0910/item.csv")
