...
def get_common_search_config():
    ...
    parser.add_argument("--dataset", type=str, default='Assistment', help="dataset for evaluation",choices=['Assistment','junyi','slp'])
    # base model setting
    ...
    parser.add_argument("--Num-Nodes", type=list, default=[2,4], help='dropout rate for FFN and self-attention')
    ...
    # Saving DIR setiing
    parser.add_argument('--exp-name', type=str, default="./experiment/Search", help="experiment name")
    ...
    config = eval("get_{}_search_config".format(config.dataset))(config)
    # device
...
def get_Assistment_search_config(config):
    config.epochs = 30
    config.lr = 0.002
    config.wd = 0.0
    return config
