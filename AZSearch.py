...
from Tree.Genetic_Operator import  Generate_AZ
...
class AZSearch():
    ...
    def Initialization(self):
        if config.Continue_path is None:
            ...
            for idx in range(0,self.Popsize):
                num_nodes = np.random.randint(config.Num_Nodes[0],config.Num_Nodes[1]) #
                self.Population.append(Individual(num_Nodes=num_nodes,mapping=self.mapping,config=self.config,gen=0,id=idx))
            ...
            self.gen = int(latest_file_or_folder[-4:])
    ...
    def Genetic_operation(self):
        ...
        Selected = np.random.choice(self.Popsize,T)
        Selected_gen = [self.Population[i].gen for i in Selected]
        Delete_index = Selected[np.argmin(Selected_gen)]

        Selected_fit =  self.Pop_fitness[Selected]
        Selected_fit = Selected_fit[:,1]

        index_generate = Selected[np.argmin(Selected_fit)]


        Selected_fit_best = self.Population[index_generate]



        self.offspring = []
        offspring_i_Dec = Generate_AZ(Selected_fit_best,self.gen)
        offspring_i = Individual(Dec=offspring_i_Dec,mapping=self.mapping,config=self.config,gen=self.gen,id=0)
        self.offspring.append(offspring_i)


        offspring_i_fintess = self.Evaluation(self.offspring)

        self.Population[Delete_index] = offspring_i
        self.Pop_fitness[Delete_index] = offspring_i_fintess
    ...
    def First_Selection(self):
        ...
        self.select_index  = np.argsort(FunctionValue.T,1)[0,:100]
        self.Population =  [Population[i] for i in self.select_index]
        self.Pop_fitness = FunctionValue[self.select_index]
    ...
    def Main_Loop(self):
        ...
        self.Maxi_Gen = (self.Maxi_Gen-1)*self.Popsize
